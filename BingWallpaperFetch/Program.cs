...
namespace BingWallpaperFetch
{
    class Program
    {
        ...
        public static string[] regionId = new string[] {"cn","tw","jp","us"};
        ...
        public static bool WallpaperWebFetch(int idx,ref Wallpaper[] wallpaperFetched,string[] regionIds)
        {
            try
            {
                ...
                for(int i = 0;i < regionIds.Length; i++,countNewWallpaper++)
                {
                    ...
                    if (regionIds[i] == "cn")
                    {
                        response = ("https://cn.bing.com/HPImageArchive.aspx?format=js&idx=" + idx + "&n=1&ensearch=0").WithHeader("X-Forwarded-For", "220.181.38.148").WithCookie("ENSEARCH", "BENVER=1").GetStringAsync().Result;
                    }
                    else
                    {
                        response = ("https://cn.bing.com/HPImageArchive.aspx?format=js&idx=" + idx + "&n=1&cc=" + regionIds[i]).WithHeader("X-Forwarded-For", "64.233.161.2").WithCookie("ENSEARCH", "BENVER=1").GetStringAsync().Result;
                    }
                    ...
                    for(int j = 0;j < i; j++)
                    {
                        ...
                        .IndexOf("_") - ((string)images[0]["urlbase"])
                        .IndexOf("OHR."));
                    ...
                    wallpaperFetched[countNewWallpaper].compareString = wallpaperFetched[countNewWallpaper].urlBase
                                                                                                           ...
                                                                                                           .IndexOf("_") - (wallpaperFetched[countNewWallpaper].urlBase)
                                                                                                           .IndexOf("OHR."));
            ...
            {
                ShowPopUpNotification("[WebRequest]Fail to fetch wallpaer and create an object", "Error", ToolTipIcon.Error);
            ...
            if (File.Exists(comparePoolPath) == true)
            {
                try
                {
                    ...
                    for (int i = 0; i < ((JArray)comparePool["items"]).Count; i++)
                    {
                        ...
                        Console.WriteLine();
                        TimeSpan itemSpan = Convert.ToDateTime(DateTime.Now).Subtract(DateTime.ParseExact((string)((JArray)comparePool["items"])[i]["fetchDate"], "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture));
                        if (itemSpan.Days >= 10)
                        {
                            continue;
                        }
        ...
        public static bool DeleteSpecificFile(string fileName,string filePathWithoutSolution)
        {
            try
            {
                ...
                {
                    File.Delete(filePathWithoutSolution + resolutions[i] + "\\" + "[" + resolutions[i] + "]" + fileName);
                }
        ...
        public static bool TrimWallpaper(ref Wallpaper[] wallpapers)
        {
            try
            {
                ...
                {
                    if (wallpapers[i].compareString != "" && wallpapers[i].compareString != null)
                    {
                        countValidWallpaper++;
                    }      
                }
            ...
            {
                ShowPopUpNotification("[Trim Wallpaper]Trim the new fetched wallpaper failed", "Error", ToolTipIcon.Error);
                return false;
            }
            return true;
        }
        ...
        public static bool WriteComparePool(Wallpaper[] newWallpapers,Wallpaper[] oldWallpapers,string comparePoolPath)
        {
            try
            {
                ...
                dynamic newComparePool = new JObject { { "Version", Application.ProductVersion.ToString() } };
            ...
            {
                ShowPopUpNotification("[Write ComaprePool]Construct and write comaprePool error", "Error", ToolTipIcon.Error);
                return false;
            }
            return true;
        }
        static void Main(string[] args)
        {
            ...
            if (args.Length == 2)
            {
                if ((args[0] == "/delete" || args[0] == "-delete") && args.Length == 2 && (args[0] != "-idx" || args[0] != "/idx") == true)
                {
                    DeleteSpecificFile(args[1], config.wallpaperFileRootPath);
                    return;
                }
                if ((args[0] == "/idx" || args[0] == "-idx") && args.Length == 2 && (args[0] != "-delete" || args[0] != "/delete") == true && args[1].Length < 3)
                {
            ...
            File.Copy(config.wallpaperFileRootPath + "compare.pool", config.wallpaperFileRootPath + "compare.pool.bak", true);
            TrimWallpaper(ref localWallpapers);
            if(WriteComparePool(newWallpapers,localWallpapers, config.wallpaperFileRootPath + "compare.pool") == true)
            {
            ...
            else
            {
                ...
                File.Move(config.wallpaperFileRootPath + "compare.pool.bak", config.wallpaperFileRootPath + "compare.pool");
            }
            ShowPopUpNotification("Done", "BingWallpaperFetch", ToolTipIcon.Info);
        }
    }
}
//最后编译通过时间：201910312131
